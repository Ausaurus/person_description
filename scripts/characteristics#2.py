...
# ----------------------------- ROS SETUP -----------------------------
class PersonCharacteristicsDetector:
    def __init__(self):
        rospy.init_node('person_characteristics_detector', anonymous=True)

        # Publishers and Subscribers
        self.pub = rospy.Publisher('/person_description', String, queue_size=10)
        rospy.Subscriber('/enable_detection', Bool, self.detection_control_callback)
        rospy.Subscriber("/camera_control", String, self.control_callback)
        ...
        rospy.loginfo("🎥 Person characteristics detector started")
    ...
    def image_callback(self, msg):
        ...
        if characteristics_detected and self.detection_enabled:
...
if __name__ == '__main__':
    try:
        detector = PersonCharacteristicsDetector()
        rospy.spin()
    except rospy.ROSInterruptException:
        pass
    finally:
        cv2.destroyAllWindows()
        rospy.loginfo("🎥 Person characteristics detector stopped")
